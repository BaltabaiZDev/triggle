...
class LanRoomBrowser {
  ...
  void _onSocketEvent(RawSocketEvent event) {
    ...
    var changed = false;
    while ((datagram = _socket?.receive()) != null) {
      final room = LanDiscoveryService.decodeAdvertisement(datagram!.data);
      if (room == null ||
          room.protocolVersion != LanEnvelope.currentProtocolVersion) {
        continue;
      }
      _recordRoom(room, persistent: false);
      changed = true;
    }
    if (changed) {
  ...
  void _onBonjourService(nsd.Service service, nsd.ServiceStatus status) {
    ...
    _recordRoom(room, persistent: true);
    _controller.add(currentRooms);
  }

  void _recordRoom(LanRoomAdvertisement room, {required bool persistent}) {
    final wasPersistent = _rooms[room.roomId]?.persistent ?? false;
    _rooms[room.roomId] = _SeenRoom(
      room,
      DateTime.now(),
      persistent: persistent || wasPersistent,
    );
  }
  ...
  void _prune() {
    ...
    _rooms.removeWhere(
      (_, seen) => !seen.persistent && seen.lastSeen.isBefore(cutoff),
    );
...
class _SeenRoom {
  const _SeenRoom(this.room, this.lastSeen, {required this.persistent});
